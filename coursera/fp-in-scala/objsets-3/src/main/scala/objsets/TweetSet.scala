...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def size(acc: Int): Int = acc + left.size(0) + right.size(1)

  override def union(that: TweetSet): TweetSet = (left union (right union that)).incl(elem)

  override def mostRetweeted: Tweet = {
    val united = (right union left).size
    if (united == 0) {
      return elem
    }
    val topRetweeted = (right union left).mostRetweeted
    if (elem.retweets >= topRetweeted.retweets) return elem
    topRetweeted
  }
